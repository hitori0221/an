...
export default function ModemsDataTable({
  ...
  const [pendingDeleteModem, setPendingDeleteModem] = useState<Modem | null>(
    null,
  );
  ...
  const handleDeleteModem = async () => {
    ...
    if (!response.ok) {
      toast.error(await getErrorMessage(response, "Unable to delete modem"));
      return;
    }

    updateModems((currentModems) =>
      currentModems.filter(
        (currentModem) => currentModem.id !== pendingDeleteModem.id,
      ),
    );
  ...
  const handleCreateModem = async (modem: Modem) => {
    ...
    if (!response.ok) {
      toast.error(await getErrorMessage(response, "Unable to create modem"));
      return;
    }
  ...
  const handleUpdateModem = async (updatedModem: Modem) => {
    ...
    if (!response.ok) {
      toast.error(await getErrorMessage(response, "Unable to update modem"));
      return;
    }
    ...
    updateModems((currentModems) =>
      currentModems.map((modem) =>
        modem.id === savedModem.id ? savedModem : modem,
      ),
    );
  ...
  return (
    <>
      ...
      />
      <ModemModal
        key={editingModem?.id ?? "edit-modem"}
        modem={editingModem}
        ...
        <DialogContent className="max-w-[420px]">
          ...
          <DialogFooter>
            <Button
              ...
              variant="ghost"
              size="sm"
              onClick={() => setPendingDeleteModem(null)}
            >
              Cancel
            </Button>
            <Button
