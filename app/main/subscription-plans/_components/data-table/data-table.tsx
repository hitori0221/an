...
import {
  ...
  subscriptionPlanColumnClassNames,
} from "./columns";
import type {
  SubscriptionPlan,
...
export default function SubscriptionPlansDataTable({
  ...
  const [subscriptionPlans, setSubscriptionPlans] =
    useState<SubscriptionPlan[]>(initialPlans);
  ...
  const updatePlans = (
    getPlans: (currentPlans: SubscriptionPlan[]) => SubscriptionPlan[],
  ) => {
      ...
      return {
        ...
        categoryIcons:
          group?.categoryIds
            ...
            .filter((category): category is SubscriptionPlanCategoryIcon =>
              Boolean(category),
            ) ?? [],
      };
    }
  ...
  const columns = getSubscriptionPlanColumns({
    ...
    onDelete: async (plan) => {
      ...
      setEditingPlan((currentPlan) =>
        currentPlan?.id === plan.id ? null : currentPlan,
      );
    },
  });
  ...
  const handleUpdatePlan = async (updatedPlan: SubscriptionPlan) => {
    const response = await fetch(
      `/api/subscription-plans/plans/${updatedPlan.id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    updatePlans((currentPlans) =>
      currentPlans.map((plan) =>
        plan.id === savedPlan.id ? withCategoryIcons(savedPlan) : plan,
      ),
    );
    setEditingPlan(null);
  };
