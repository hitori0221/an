...
function getInitialForm(plan: SubscriptionPlan | null) {
  return plan
    ? {
        code: plan.code,
        name: plan.name,
        planTarget:
          plan.categoryType === "group" && plan.groupId
            ? `group:${plan.groupId}`
            : plan.categoryId
...
export function EditPlanDrawer({
  ...
  const handleSubmit = () => {
    ...
    const [targetType, targetId] = form.planTarget.split(":") as [
      "category" | "group",
      string,
    ];
      ...
      <SheetContent className="w-full sm:w-[420px]" side="right">
        <SheetHeader className="pr-12">
          <div className="min-w-0">
            ...
            <SheetDescription>
              Update the plan details used for subscriber assignment, import,
              and export.
            </SheetDescription>
          </div>
        </SheetHeader>
