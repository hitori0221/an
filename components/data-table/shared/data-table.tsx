...
const EMPTY_TABLE_MESSAGE = "Nothing to see here… yet. Stay tuned!";
...
export function DataTable<TData, TValue>({
  ...
  useLayoutEffect(() => {
    const updatePageSize = () => {
      ...
      const pageSize = Math.max(
        MIN_PAGE_SIZE,
        Math.floor(availableHeight / TABLE_ROW_HEIGHT),
      );
  ...
  return (
    <div ref={rootRef} className="min-w-0 w-full">
      ...
      >
        <Table
          className={cn("table-fixed", data.length > 0 && minWidthClassName)}
        >
              ...
              <TableRow key={headerGroup.id} className="hover:bg-transparent">
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className={cn(
                      "h-9 overflow-hidden border-b px-3",
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              ...
              <TableRow>
                ...
                >
                  {globalFilter
                    ? "Zero records found. It’s suspiciously peaceful here."
                    : EMPTY_TABLE_MESSAGE}
                </TableCell>
              </TableRow>
            )}
