...
export const subscriberColumns: ColumnDef<Subscriber>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className='font-mono text-xs text-muted-foreground'>{row.getValue('phoneNumber')}</span>
    ),
  },
  {
    ...
    accessorKey: 'plan',
    cell: ({ row }) => (
      <span className='inline-flex h-6 items-center rounded-md bg-muted/70 px-2 text-xs font-medium text-foreground'>
        {row.getValue('plan')}
      </span>
    ),
  },
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className='text-muted-foreground'>{row.getValue('barangay')}</span>
    ),
  },
  {
