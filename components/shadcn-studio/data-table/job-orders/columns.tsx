...
export const jobOrderColumns: ColumnDef<JobOrder>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className='flex items-center gap-2 font-medium'>
        ...
        <span className='font-mono text-[13px] text-foreground'>{row.getValue('ticketNumber')}</span>
      </div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span className='block truncate font-medium text-foreground'>{row.getValue('subscriberName')}</span>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span className='block truncate text-muted-foreground'>{row.getValue('problemCategory')}</span>
    ),
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          variant='outline'
          className={cn('px-2 py-0.5 font-medium', priorityStyles[priority])}
        >
          {priority}
        </Badge>
      )
