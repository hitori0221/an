...
alter table public.profiles add constraint profiles_permission_check check (permission in ('administrator', 'billing_management', 'operations')),
  add constraint profiles_billing_branch_check check (permission <> 'billing_management' or branch_id is not null);
